    ...
    while True:
        ...
        try:
            if (init_config and protocol.state == ConnectionState.CONNECTED):
                ...
                protocol.send_cmd("mac set ar " + config['ar'])
                protocol.send_cmd("mac set pwridx %d" % config['pwridx'])
                protocol.send_cmd("mac set class " + config['class'])
                protocol.send_cmd("mac set retx %d" % config['retx'])
                protocol.send_cmd("mac set adr " + config['adr'])
                logging.info("Done...")
                init_config = 0
